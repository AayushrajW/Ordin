...
Nothing here has an answer in this build. Each is accepted deliberately, with what would
fix it.

| # | Risk | Why accepted | What would fix it |
|---|---|---|---|
| **AR-1** | **No real authentication.** The subject is a declared stub. | 3–4 days, solo. Building an identity provider consumes the differentiator budget entirely. | A real IdP behind the same interface. The stub names it as its `production_adapter` and is never described as an auth service. |
...
| **AR-9** | **No rate limiting, lockout or user-enumeration protection.** | Unowned by any slice; the demo is driven from the host. **Not acceptable for a pilot.** | A database-backed attempt counter, uniform login responses, and binding published ports to localhost. |
...
| **AR-14** | **No revocation propagation and no way to answer "who currently has access to this case, and why".** | Administration screens are explicitly out of scope, protecting differentiator time. Expiry does most of the work: grants die by themselves. | A read-only current-access panel. State the bound instead: revocation takes effect within one request, no decision is cached across requests, and delivered bytes are unrecoverable by any design. |
