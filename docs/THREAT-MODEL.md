...
| ID | Actor → goal | Vector | Control · maturity · slice | Residual |
|---|---|---|---|---|
| **EXT-01** | Anyone reaching the API port → read case records with no credential | ~~No authentication slice exists~~ **CLOSED** (ADR 0024): password login with Argon2id, account lockout counted on the row, and the subject re-resolved server-side from a signed session on every request — never from a header | **PLAN adds `SimulatedSubjectProvider`, subject resolved server-side from a signed session** · `mvp` · 3 | Declared stub, not an identity provider. Names its `production_adapter`. See [AR-1](#accepted-risks). |
...
| **VIC-06** | Derivative-only role → fetch the page preview instead of the document | ~~served from their own route with its own weaker check~~ **CLOSED 2026-09-26**: there is no separate thumbnail route. `/versions/{id}/page.png` renders server-side and `/versions/{id}/pages` reports the page count, and both resolve through `_resolve_version` — the same membership test against `readable_version_ids` that the document routes use, so a grantee can name only their own derivative | `covered` | Every preview must be a version-scoped derivative subject to the same policy, and previews of a redacted document must render from the derivative. |
| **VIC-07** | Ongoing case reader, after lawful disposal → the identity of a victim whose record was destroyed | ~~OCR text, extracted field values and the index are untouched and fully queryable~~ **NARROWED 2026-09-26** (ADR 0031): `POST /versions/{id}/dispose` deletes the stored bytes, `ocr_text`, `ocr_word` and `extracted_field` in one transaction, which also removes the document from search — the index is a functional GIN over `ocr_text`. `verify()` returns `DISPOSED_ANCHOR_ONLY`. What is NOT closed: irreversibility. Write-ahead logs, snapshots and any backup taken before the disposal are out of reach of this code, and there is no key-per-version crypto-erase — so the claim is "the stored original is destroyed", never "the document is destroyed" | `partial` | **The gap between "we destroyed the document" and "we destroyed the record"**, and a plausible question from a Women Safety panel. Disposal must cascade to derived text. |
...
|---|---|---|---|
| **AZM-01** | ~~**Nothing establishes the subject.**~~ **CLOSED** (ADR 0024): real credentials, and the subject is re-resolved from the database on every request, so a lapsed clearance or a transfer takes effect immediately rather than at the end of a session | `SimulatedSubjectProvider`, server-side session · `mvp` · 3 | See EXT-01, [AR-1](#accepted-risks). |
...
| **AR-13** | ~~**Disposal does not reach derived text**~~ **NARROWED 2026-09-24** (ADR 0031): `POST /versions/{id}/dispose` deletes the stored bytes, `ocr_text`, `ocr_word` and `extracted_field` in one transaction, which also removes the document from search (the index is a functional GIN over `ocr_text`). It refuses on an unanchored version, and it keeps the bytes when another surviving version shares the same content address. | The **irreversibility half is unchanged**: write-ahead logs, filesystem snapshots and any backup taken before the disposal are out of reach of this code, and there is no key-per-version crypto-erase. | Scope keys per version so erasure has a bounded radius. **The wording the UI and the API now use is "the stored original is destroyed", never "the document is destroyed"** — and where the bytes are deliberately retained for a shared address, the response says so instead. |
...
| **AR-17** | ~~**Bulk export is unmetered.**~~ **NARROWED 2026-09-24** (ADR 0031): export is a distinct verb. `GET /versions/{id}/export` and `GET /cases/{id}/export` each write an `export_record` row — scope, disclosure class, row count, byte count, excluded count, all enumerated or numeric, no free text — and an audit row. Both sit in their own rate-limit bucket (20/min), and a case export refuses above 50 documents or 32 MB. | **Accounting, not prevention**, exactly as this row always said. A designated officer can still export every case they hold; the change is that the question "how much left, under whose name, through which disclosure class" now has an answer. Cross-case list and search remain unmetered. | Quotas and volume anomaly detection over `export_record`, which now holds the data they would need. |
...
| **AR-18** | ~~**No backups and no restore verification**~~ **NARROWED 2026-09-21** (ADR 0027): `scripts/backup.sh` dumps the database then archives the blob volume, in that order, and `scripts/restore.sh` reverses it behind an explicit confirmation. `fresh` is still a one-command total wipe, by design. | The **verification half is still open**: `restore.sh` has not been replayed onto a scratch host, and a backup nobody has restored is a belief rather than a backup. The script says so in its own output. | Run the restore once against a scratch host, after the last good rehearsal. |
